...
const inputString: string = fs.readFileSync("day9/day9_input1.txt", "utf-8");
...
function floodfill(y: number, x: number, checkMap: number[][]) {
...
function calcBasinSize(y: number, x: number): number {
    let checkMap: number[][] = Array(length_i)
        .fill(0)
    ...
    let size = checkMap.reduce(
        (total, y) =>
            total +
            y.reduce(
                (counter, markedElement) =>
                    markedElement === 1 ? counter + 1 : counter,
                0
            ),
        0
    );
...
// let largestBasinsSize: number[] = Array(3).fill(0); // another option to find 3 largest basins
