// implemented in js only, had no access to the typescript compiler

var fs = require("fs");
var text = fs.readFileSync("./input.txt").toString("utf-8");

let [dotsString, inputString] = text
    .split("\n\n")
    .map((str) => str.split("\n"));

let dots = dotsString.map((coordinate) =>
    coordinate.split(",").map((el) => parseInt(el, 10))
);
let instructions = [];
inputString.forEach((str) => {
    let equalsSignIndex = str.indexOf("=");
    instructions.push([
        str.charAt(equalsSignIndex - 1),
        parseInt(str.substring(equalsSignIndex + 1), 10),
    ]);
...
instructions.forEach((foldingInstruction) => {
    ...
    dots = dots.filter((coordinate, idx) => {
        if (coordinate[foldingAxis] > foldingLineNumber) {
            const newCoord =
                foldingLineNumber -
                (coordinate[foldingAxis] - foldingLineNumber);
...
let matrix = Array(ySize)
    .fill(" ")
    .map((y) => Array(xSize).fill(" "));
